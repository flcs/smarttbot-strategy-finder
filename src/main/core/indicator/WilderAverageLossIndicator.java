...
public class WilderAverageLossIndicator extends AbstractIndicator<Decimal> {
	...
	public WilderAverageLossIndicator(Indicator<Decimal> indicator, int timeFrame) {
	...
	public Decimal getValue(int index) {
		...
		currentIndex = index;
		lastEma = ema;
		ema = ema.multipliedBy(Decimal.valueOf(timeFrame - 1)).plus(getLoss(index))
				.dividedBy(Decimal.valueOf(timeFrame));
		return ema;
	}

	public Decimal getLoss(int index) {
		Decimal change = indicator.getValue(index - 1).minus(indicator.getValue(index));
